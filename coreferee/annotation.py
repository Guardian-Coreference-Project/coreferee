...
from .data_model import Mention, Chain, FeatureTable
...
class Annotator:
    ...
    @staticmethod
    def record_mention(preceding_mention:Mention, token:Token,
            token_indexes_without_coordination_to_mention_sets:dict,
            token_indexes_with_coordination_to_mention_sets:dict) -> None:
        ...
        if len(preceding_mention.token_indexes) > 1:
            if preceding_mention.root_index in token_indexes_with_coordination_to_mention_sets:
                ...
                for token_index in preceding_mention.token_indexes:
                    token_indexes_with_coordination_to_mention_sets[token_index] = mention_set
        else:
            preceding_token = token.doc[preceding_mention.root_index]
            if preceding_token.i in token_indexes_without_coordination_to_mention_sets:
                mention_set = token_indexes_without_coordination_to_mention_sets[preceding_token.i]
            else:
                mention_set = {preceding_mention}
                token_indexes_without_coordination_to_mention_sets[preceding_token.i] = mention_set
        mention_set.add(Mention(token, False))
        if token.i in token_indexes_without_coordination_to_mention_sets:
            mention_set.update(token_indexes_without_coordination_to_mention_sets[token.i])
            for mention in token_indexes_without_coordination_to_mention_sets[token.i]:
                token_indexes_without_coordination_to_mention_sets[mention.root_index] = mention_set
        else:
    ...
    def get_compatibility(self, token:Token, mention_set:set) -> int:
        ...
        for mention in mention_set:
            if self.rules_analyzer.is_independent_noun(token.doc[
                    mention.root_index]):
                mention_set_contains_referring_mention = True
            working_result = self.rules_analyzer.is_potential_anaphoric_pair(mention, token, False)
    ...
    def temp_annotate_any_coreferring_noun_link(self, token:Token, sentence_deque:deque,
            token_indexes_without_coordination_to_mention_sets:dict,
            token_indexes_with_coordination_to_mention_sets:dict) -> None:
        doc = token.doc
        ...
        for sent in sentence_deque:
            for preceding_token in (doc[index] for index in range(sent.end, sent.start-1, -1)
                    if index < token.i):
                if preceding_token._.coref_chains.temp_potentially_referring and \
                        self.rules_analyzer.is_potential_coreferring_noun_pair(preceding_token,
                        token):
                    self.record_mention(Mention(preceding_token, False), token,
                ...
                if preceding_token.i in token_indexes_without_coordination_to_mention_sets:
                    ...
                    mention_set = \
                        token_indexes_without_coordination_to_mention_sets[preceding_token.i]
                    for mention in (mention for mention in mention_set if
                            len(mention.token_indexes) == 1):
    ...
    def temp_annotate_any_anaphoric_link(self, token:Token,
            token_indexes_without_coordination_to_mention_sets:dict,
            token_indexes_with_coordination_to_mention_sets:dict,
            permitted_start_index:int=0) -> bool:
        """ Returns *True* if an annotation occurred. """

        def check_mention_sets_for_reflexive_relationships(mention:Mention,
                index_to_mention_set_dict:dict) -> bool:
            for token_index in mention.token_indexes:
                if token_index in index_to_mention_set_dict:
                    for working_mention in index_to_mention_set_dict[token_index]:
                        if self.rules_analyzer.is_potential_reflexive_pair(working_mention, token):
                            return True
            return False

        def intern_temp_annotate_any_anaphoric_link(allow_uncertainty:bool) \
                -> bool:
            for index, potential_referred in \
                    enumerate(token._.coref_chains.temp_potential_referreds):
                if index < permitted_start_index or index >= self.RETRY_DEPTH:
                    continue
                if len(potential_referred.token_indexes) == 1:
                    if potential_referred.root_index in \
                            token_indexes_without_coordination_to_mention_sets:
                        mention_set = \
                            token_indexes_without_coordination_to_mention_sets[
                            potential_referred.root_index]
                        compatibility = self.get_compatibility(token, mention_set)
                        if compatibility == 0 or (compatibility == 1 and not allow_uncertainty):
                            continue
                if self.rules_analyzer.is_reflexive_anaphor(token) == 0 and (
                        check_mention_sets_for_reflexive_relationships(potential_referred,
                        token_indexes_without_coordination_to_mention_sets) or \
                        check_mention_sets_for_reflexive_relationships(potential_referred,
                        token_indexes_with_coordination_to_mention_sets)):
                    continue
                self.record_mention(potential_referred, token,
                    token_indexes_without_coordination_to_mention_sets,
                    token_indexes_with_coordination_to_mention_sets)
                return True
            return False
    ...
    def delete_from_collections_for_rewind(self, previous_token:Token, token:Token,
            token_indexes_without_coordination_to_mention_sets:dict,
            token_indexes_with_coordination_to_mention_sets:dict) -> None:

        def intern_delete_from_collections_for_rewind(dictionary:dict, working_token:Token):
        ...
        for working_token in doc[previous_token.i: token.i+1]:
            intern_delete_from_collections_for_rewind(
                token_indexes_without_coordination_to_mention_sets, working_token)
            intern_delete_from_collections_for_rewind(
    ...
    def attempt_rewind_with_previous_token_and_retry_index(
        ...
        if self.temp_annotate_any_anaphoric_link(previous_token,
            ...
            for working_token in doc[previous_token.i+1: token.i+1]:
                self.temp_annotate_any_coreferring_noun_link(working_token, sentence_deque,
                    token_indexes_without_coordination_to_mention_sets,
                    token_indexes_with_coordination_to_mention_sets)
                if hasattr(working_token._.coref_chains, 'temp_potential_referreds'):
                    if not self.temp_annotate_any_anaphoric_link(working_token,
                            token_indexes_without_coordination_to_mention_sets,
                            token_indexes_with_coordination_to_mention_sets):
                        return False
            return True
    ...
    def attempt_retry(self, token:Token, coreferring_deque:deque, sentence_deque:deque,
        ...
        previous_token = None
        for retry_index in range(1, min(self.RETRY_DEPTH,
            ...
            for previous_token in (t for t in coreferring_deque if
                ...
                self.delete_from_collections_for_rewind(previous_token, token,
                    token_indexes_without_coordination_to_mention_sets,
                    token_indexes_with_coordination_to_mention_sets)
                if self.attempt_rewind_with_previous_token_and_retry_index(retry_index,
                        previous_token, token, sentence_deque,
                        token_indexes_without_coordination_to_mention_sets,
                        token_indexes_with_coordination_to_mention_sets):
                    return True
        if previous_token is not None:
            ...
            self.delete_from_collections_for_rewind(previous_token, token,
                token_indexes_without_coordination_to_mention_sets,
                token_indexes_with_coordination_to_mention_sets)
            self.attempt_rewind_with_previous_token_and_retry_index(0, previous_token, token,
                sentence_deque, token_indexes_without_coordination_to_mention_sets,
                token_indexes_with_coordination_to_mention_sets)
        return False
    ...
    def get_most_specific_mention(self, mention_set:set, doc:Doc):
        ...
        for mention in mention_set:
            ...
            if self.rules_analyzer.is_independent_noun(
                    this_mention_root_token) and not \
                    self.rules_analyzer.is_independent_noun(
                    stored_mention_root_token):
                stored_mention = mention
            if this_mention_root_token.pos_ in self.rules_analyzer.propn_pos and \
                    this_mention_root_token.ent_type_ != '' and (
                    stored_mention_root_token.pos_ not in self.rules_analyzer.propn_pos or
                    this_mention_root_token.ent_type_ == ''):
                stored_mention = mention
    ...
    def annotate(self, doc:Doc, used_in_training=False):
        ...
        sentence_deque = deque(
            maxlen=self.rules_analyzer.maximum_coreferring_nouns_sentence_referential_distance + 1)
        coreferring_deque = deque(maxlen=self.RETRY_DEPTH)
        for sent in doc.sents:
            sentence_deque.appendleft(sent)
            for token in sent:
                self.temp_annotate_any_coreferring_noun_link(token, sentence_deque,
                    token_indexes_without_coordination_to_mention_sets,
                    token_indexes_with_coordination_to_mention_sets)
                if hasattr(token._.coref_chains, 'temp_potential_referreds'):
                    if self.temp_annotate_any_anaphoric_link(token,
                            token_indexes_without_coordination_to_mention_sets,
                            token_indexes_with_coordination_to_mention_sets) or \
                            self.attempt_retry(token,
                            coreferring_deque, sentence_deque,
                            token_indexes_without_coordination_to_mention_sets,
                            token_indexes_with_coordination_to_mention_sets):
        ...
        chains = []
        for token_index, mention_set in token_indexes_without_coordination_to_mention_sets.items():
            if token_index in visited_token_indexes:
                continue
            mention_list = sorted(list(mention_set), key=lambda mention: mention.root_index)
            most_specific_mention = self.get_most_specific_mention(mention_list, doc)
            chain = Chain(mention_list, mention_list.index(most_specific_mention))
            chains.append(chain)
        ...
        for index, chain in enumerate(chains):
            chain.index = index
            for mention in chain.mentions:
                for token in (doc[token_index] for token_index in mention.token_indexes):
        ...
        if not used_in_training:
            ...
            for temp_entry in [t for t in doc._.coref_chains.__dict__ if t.startswith('temp_')][:]:
